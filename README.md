...
> **A note on discussions labelled as archived**: We label some discussions with an `archive` label. This is because sometimes we find two or more discussions open around the same topic. Currently, you cannot merge two discussions into one, so our current workaround is to link to the least active discussion(s) from the most active. We will lock the older discussions and add the `archived` label. This allows us to keep a record of the older discussions but focus on, the more active discussion.
